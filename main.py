...
def train_step(anchor, positive, negative, model, criterion, optimizer):
    ...
    negative_distance = L1Dist()(anchor_output, negative_output)

    positive_labels = torch.ones(positive_distance.size(), device=device)
    negative_labels = torch.zeros(negative_distance.size(), device=device)

    positive_loss = criterion(positive_distance, positive_labels)
    negative_loss = criterion(negative_distance, negative_labels)
...
def train(model, train_loader, num_epochs=50, lr=0.00001, patience=5):
...
class SiameseDataset(Dataset):
    def __init__(self, anchor_dir, positive_dir, negative_dir, transform=None):
        ...
        self.negative_images = random.sample(negative_images, 32000)
...
anchor_dir = r'C:\Users\Administrator\Desktop\face_recognition2_finished\data\anchor'
positive_dir = r'C:\Users\Administrator\Desktop\face_recognition2_finished\data\positive'
negative_dir = r'C:\Users\Administrator\Desktop\face_recognition2_finished\data\negative'
